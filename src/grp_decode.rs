...
impl<'a> GrpHeader<'a> {
    /// callback is x, y, color
    /// x/y are relative to frame topleft, not sprite.
    fn decode<F: FnMut(usize, usize, u8)>(&self, mut set_color: F) -> Result<(), Error> {
        let width = self.frame_width as usize;
        for y in 0..(self.frame_height as usize) {
            let line_offset = self.frame_lines.get((y * 2)..)
            ...
            while x < width {
                ...
                if byte & 0x80 != 0 {
                    // Transparent
                    x += (byte & 0x7f) as usize;
                    line_data = rest;
                } else if byte & 0x40 != 0 {
                    // memset
                    let length = (byte & 0x3f) as usize;
                    let length = (width - x).min(length);
                    let end = x + length;
                    ...
                    let data = rest.get(..length)
                        .ok_or_else(|| Error)?;
                    for (x, color) in (x..end).zip(data.iter().copied()) {
                        set_color(x, y, color);
                    }
...
pub fn decode_grp_to_rgba(grp: &[u8], frame: u16, palette: &[u8]) -> Result<GrpFrame, Error> {
    ...
    let start_pos = header.x as usize + header.y as usize * header.height as usize;
    header.decode(|x, y, color| {
        let pos = start_pos
            .wrapping_add(x)
            .wrapping_add(y.wrapping_mul(header.height as usize))
            .wrapping_mul(4);
        let color = &palette[(color as usize * 4)..][..4];
        (&mut buffer[pos..(pos.wrapping_add(4))]).copy_from_slice(color);
    })?;
    Ok(GrpFrame {
        data: buffer,
        left: header.x.into(),
...
pub fn decode_grp_to_rgba_and_teamcolor(
    ...
    header.decode(|x, y, color| {
        ...
        if color >= 8 && color < 0x10 {
            ...
            let strength = (((1.0 - ((color - 8) as f32) / 9.0)) * 255.0) as u8;
            for i in 0..3 {
                buffer[pos + i] = strength;
                teamcolor[pos + i] = 255;
            }
            buffer[pos + 3] = 255;
            teamcolor[pos + 3] = 255;
